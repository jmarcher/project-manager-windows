...
namespace GUI.Utils
{
    ...
    public class ListViewColumnSorter : IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            ...
            compareResult = CompararObjetosPorTipoColumna(listviewX, listviewY);
        ...
        private int CompararObjetosPorTipoColumna(ListViewItem listviewX, ListViewItem listviewY)
        {
            if (listviewX.SubItems[ColumnToSort].Tag != null)
            {
                if (listviewX.SubItems[ColumnToSort].Tag.Equals("int"))
                {
                    return Int32.Parse(listviewX.SubItems[ColumnToSort].Text).CompareTo(Int32.Parse(listviewY.SubItems[ColumnToSort].Text));
                }
                else if (listviewX.SubItems[ColumnToSort].Tag.Equals("string"))
                {
                    return String.Compare(listviewX.SubItems[ColumnToSort].Text, listviewY.SubItems[ColumnToSort].Text);
                }
                else if (listviewX.SubItems[ColumnToSort].Tag.Equals("DateTime"))
                {
                    return DateTime.Compare(CrearDateTimeDesdeString(listviewX.SubItems[ColumnToSort].Text),
                        CrearDateTimeDesdeString(listviewY.SubItems[ColumnToSort].Text));
                }
            }
                return ObjectCompare.Compare(listviewX.SubItems[ColumnToSort].Text, listviewY.SubItems[ColumnToSort].Text);
        }

        private DateTime CrearDateTimeDesdeString(string p)
        {
